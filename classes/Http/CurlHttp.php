 ...
 *
 * - **HTTPS only.** `CURLPROTO_HTTPS` on the request and on its redirects. The
 *   server token travels in a header on every one of these calls, and sending
 *   it over plain HTTP once is sending it for good.
 ...
 * - **A response cap.** Postmark's answers are a few kilobytes. Ten megabytes
 *   of anything is this pointed at something else, and the write callback stops
 *   reading rather than filling memory.
 * - **Short timeouts.** These run on a button press in the admin, and a
 *   settings screen that hangs for a minute because somebody else's API is slow
 *   is a settings screen a merchant reloads and presses again.
 */
final class CurlHttp implements Http
{
    ...
    public function json(string $method, string $url, ?array $body = null, array $headers = []): array
    {
        ...
        $answer = $this->run($method, $url, $payload, $headers + [
